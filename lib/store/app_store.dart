// ignore_for_file: file_names, avoid_returning_null_for_void

import 'dart:convert';

import 'package:cryptocurrency/models/currency_model.dart';
import 'package:cryptocurrency/models/default_setting.dart';
import 'package:cryptocurrency/utils/app_constant.dart';
...
abstract class AppStoreBase with Store {
  ...
  @action
  Future<void> setUseMaterialYouTheme(
    bool val, {
  ...
  @action
  Future setSelectedGraphColor(Color aColor) async =>
      mSelectedGraphColor = aColor;

  @action
  Future setSelectedMarketType(ChartMarketType aType) async =>
      mSelectedMarketType = aType;

  @action
  Future setSelectedDashboard(DefaultSetting aType) async =>
      mDashboardType = aType;

  @action
  Future setSelectedDefaultChart(DefaultSetting aType) async =>
      mDefaultChart = aType;

  @action
  Future setSelectedDefaultTrendingCard(DefaultSetting aType) async =>
      mDefaultTrendingCard = aType;

  @action
  Future setLoading(bool aIsLoading) async => mIsLoading = aIsLoading;

  @action
  Future<CurrencyModel> setSelectedCurrency(
    CurrencyModel aSelectedCurrency,
  ) async {
    setValue(
      SharedPreferenceKeys.SELECTED_CURRENCY,
      jsonEncode(aSelectedCurrency),
    );
  ...
  Future<void> setLanguage(String aCode) async {
    ...
    selectedLanguageDataModel = getSelectedLanguageModel(
      defaultLanguage: AppConstant.defaultLanguage,
    );
  ...
  @action
  Future<void> setSocialLogin(bool val, {bool isInitializing = false}) async {
    isSocialLogin = val;
    if (isInitializing) await setValue(SharedPreferenceKeys.isSocialLogin, val);
  }
  ...
  Future<void> setEmailLogin(bool value, {bool isInitializing = false}) async {
    isEmailLogin = value;
    if (isInitializing) {
      await setValue(SharedPreferenceKeys.isEmailLogin, value);
    }
  }
  ...
  Future<void> setDarkMode(bool aIsDarkMode) async {
    ...
    if (isDarkMode) {
      ...
      defaultLoaderBgColorGlobal = themePrimaryColor;
